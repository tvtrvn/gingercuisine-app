// Client leaf for the order page. The merged menu (base + owner overrides +
...
export function OrderPageClient({ items: menuItems }: { items: MenuItem[] }) {
  ...
  function getSelectedSize(item: MenuItem): SizeOption | undefined {
    ...
    return (
      item.availableSizes.find((size) => !size.soldOut) ?? item.availableSizes[0]
    );
  }
  ...
  function getSelectedAddons(item: MenuItem): AddonOption[] {
    ...
    return item.availableAddons.filter(
      (addon) => selectedIds.includes(addon.id) && !addon.soldOut,
    );
  }
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {itemSoldOut && <Badge tone="danger">Sold out</Badge>}
                        </div>
                        {item.vietnameseName && (
                    ...
                    {item.availableSizes && item.availableSizes.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {item.availableSizes.map((size) => {
                          const selected = getSelectedSize(item)?.id === size.id;
                          return (
                            <button
                              key={size.id}
                              type="button"
                    ...
                    {item.availableAddons && item.availableAddons.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {item.availableAddons.map((addon) => {
                          const selectedIds = selectedAddonsByItem[item.id] ?? [];
                          const selected =
                            selectedIds.includes(addon.id) && !addon.soldOut;
                          return (
                            <button
                              key={addon.id}
                              type="button"
